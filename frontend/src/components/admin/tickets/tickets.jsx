...
export default function AdminTickets({ account }) {
    ...
    function onMessageSubmit(message, target = account) {
                ...
                {tickets.map((item, i) => {
                    ...
                    return (
                        <section key={i} onClick={() => setTicketSelect(i)} className={`elem ${ticketSelect === i && 'selected'}`}>
                            ...
                            <div className={`lastmessage ${lastmessage.views.indexOf(account.uid) === -1 && 'new'}`}>
                                {lastmessage.text.lenght > 40 ? lastmessage.text.substring(0, 40) + '...' : lastmessage.text}
                            </div>
                        </section>
                    )
                })}
                ...
                {ticketSelect === -1 ? (
                    ...
                    <div className="wrapper">
                            ...
                            {tickets[ticketSelect].messages.map((item, i) => {
                                ...
                                return (
                                    <div key={i} className={`message ${item.sender.uid !== tickets[ticketSelect].owner.uid && 'reverse'}`}>
                                        <div className="info">
                                            <Avatar image={
                                                item.sender.uid === account.uid ? account.avatar.image
                        ...
                        <div className={`form ${(actionToggle || helpToggle) && 'helper'}`}>
                            ...
                            }} type="text" placeholder="Напишите ответ" maxLength={144} />
                        </div>
                    </div>
                )}
            </div>
